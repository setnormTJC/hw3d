...
AssTest::AssTest(Graphics& gfx, std::mt19937& rng,
	std::uniform_real_distribution<float>& adist, std::uniform_real_distribution<float>& ddist,
	...
	if (!IsStaticInitialized())
	{
		...
		const auto pModel = imp.ReadFile("models/suzanne.obj",
			...
			aiProcess_JoinIdenticalVertices); //this second param can SUBSTANTIALLY (10x) reduce vertex count
						//though some "situations"/shading goals might require independent vertices
						
		//enum flags bitwise OR'd!
		//note that ai stands for "asset import" (not artificial intelligence) 

		//pModel type is an asset import "scene" 

		const auto pMesh = pModel->mMeshes[0];
		//mMeshes is an array of aiMesh STRUCTS
		//aiMesh has about 20 PUBLIC fields and 15 public functions

		for (unsigned int i = 0; i < pMesh->mNumVertices; ++i)
		{
			vbuf.EmplaceBack(dx::XMFLOAT3
				{ pMesh->mVertices[i].x * scale, pMesh->mVertices[i].y * scale, pMesh->mVertices[i].z * scale },
				
				*reinterpret_cast<dx::XMFLOAT3*>(&pMesh->mNormals[i]) //CRAZY casting going on here 
			);
		}
		...
		for (unsigned int i = 0; i < pMesh->mNumFaces; ++i)
		{
			...
			assert(face.mNumIndices == 3); //no quads!
